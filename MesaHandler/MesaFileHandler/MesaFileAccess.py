...
class MesaFileAccess(IMesaInterface):
            ...
            for file, parameteDict in self.dataDict[section].items():
                if key in parameteDict.keys():
                    ...
                    if isinstance(value, bool):
                        ...
                        raise AttributeError("Cannot convert type " +
                                             str(type(value)) +
                                             "to known type")
    ...
    def addValue(self, key, value=None):
        ...
        if section == "":
            raise KeyError("The parameter " + key + " is not available through Mesa. \
                           Please add it to the defaults list, \
    ...
    def removeValue(self, key):
        ...
        if section == "":
            raise KeyError("The parameter " + key + " is not available through \
                            Mesa. Please add it to the defaults list before \
                            adding it to the inlist files")

        if section not in sections:
            raise ValueError("Mesa says section for parameter " + key +
                             " is " + section + ". This section is not \
                             in the sections read by this code.")
